...
class SelmaBroker:
        ...
        if file_path.endswith(".csv"):
            try:
                with open(file_path, newline='', encoding='utf-8') as csvfile:
                    ...
                    return all(header in headers for header in SelmaBrokerConfig.EXPECTED_HEADERS)
    ...
    def process_transactions(self, transactions, file_path=None):
        categorized_data = {category: [] for category in SelmaBrokerConfig.CATEGORY_MAPPING.values()}
        ...
        self._calculate_dividend_shares(categorized_data["trades"], categorized_data["dividends"])
            ...
            for key, category in SelmaBrokerConfig.CATEGORY_MAPPING.items():
                if key in description:
                    if category == "deposits_withdrawals":
                        tx["type"] = "TransferOut" if tx["total_amount"] < 0 else "TransferIn"
                        tx["holding"] = self.holding_map.get(iban, "")
                    elif category == "trades":
                        if pd.isna(tx["share_count"]) or tx["share_count"] == "":
                            logging.warning(f"Skipping trade due to invalid share count: {tx}")
                            return
    ...
    def _merge_related_data(self, categorized_data):
        ...
        if categorized_data["dividends"] and withholding_taxes:
            self._merge_withholding_taxes(categorized_data["dividends"], withholding_taxes)
    ...
    def _merge_stamp_duties(self, trades, stamp_duties):
        ...
        if not trades_df.empty and not stamp_duties_df.empty:
            stamp_duties_grouped = stamp_duties_df.groupby(['datetime', 'Fund'])['Amount'].sum().reset_index()
            trades_df = pd.merge(
                trades_df,
    ...
    def _merge_withholding_taxes(self, dividends, withholding_taxes):
        ...
        if not dividends_df.empty and not withholding_taxes_df.empty:
            withholding_taxes_df['date_range_start'] = withholding_taxes_df['datetime'] - pd.Timedelta(days=2)
            withholding_taxes_df['date_range_end'] = withholding_taxes_df['datetime'] + pd.Timedelta(days=2)

            merged_taxes = []
            for _, dividend in dividends_df.iterrows():
                matching_taxes = withholding_taxes_df[
                    (withholding_taxes_df['Fund'] == dividend['isin_code']) &
                    (withholding_taxes_df['datetime'] >= dividend['datetime'] - pd.Timedelta(days=3)) &
                    (withholding_taxes_df['datetime'] <= dividend['datetime'] + pd.Timedelta(days=3))
                ]
            ...
            if not trades_df.empty and not dividends_df.empty:
                ...
                for index, row in dividends_df.iterrows():
                    relevant_trades = trades_df[
                        (trades_df['datetime'] < row['datetime']) & (trades_df['isin_code'] == row['isin_code'])
                    ]
                    if not relevant_trades.empty:
                        ...
                        if net_shares > 0:
                            ...
                            if not pd.isna(row['total_amount']) and net_shares > 0:
                                row['price_per_share'] = row['total_amount'] / net_shares
                            else:
                                logging.warning(f"Skipping dividend with NaN price or shares: {row}")
                                continue
                        else:
                            logging.warning(f"Net shares are zero or negative for dividend: {row}")
                            continue
                    else:
                        logging.warning(f"No relevant trades found for dividend: {row}")
                        continue

                    if not (pd.isna(row.get('price_per_share')) or pd.isna(row.get('share_count'))):
                        valid_dividends.append(row)

                dividends.clear()
                dividends.extend(pd.DataFrame(valid_dividends).to_dict(orient='records'))
    ...
    def _aggregate_same_day_dividends(self, dividends):
        if dividends:
            ...
            if not dividends_df.empty:
                ...
                grouped_dividends = dividends_df.groupby(['datetime', 'isin_code', 'currency']).agg({
                    'total_amount': 'sum',
                    'share_count': 'first',
                    'broker': 'first',
                ...
                for _, row in grouped_dividends.iterrows():
                    matching_dividends = dividends_df[(dividends_df['datetime'] == row['datetime']) &
                    ...
                    if not matching_dividends.empty:
                        ...
                        row['price_per_share'] = row['total_amount'] / row['share_count'] if row['share_count'] > 0 else None
                dividends.clear()
