...
class SaxoBrokerConfig:
    ...
    def common_fields(tx, portfolio_number, holding_map):
        ...
        if category == "trade":
            transaction_date = process_datetime_to_utc(transaction_date, custom_format="%d-%b-%Y%H:%M:%S")
            datetime_obj = transaction_date
        else:
            transaction_date = process_datetime_to_utc(transaction_date)
            specific_time = SaxoBrokerConfig.CATEGORY_TIME_MAPPING.get(category, "00:00:00Z")
            datetime_obj = datetime.strptime(f"{transaction_date.date()} {specific_time}", "%Y-%m-%d %H:%M:%S%z")
...
class SaxoBroker(BaseBroker):
    ...
    def move_and_rename_file(self, file_path, transactions):
        ...
        try:
            new_path = move_file_with_conflict_resolution(file_path, SaxoBrokerConfig.TARGET_DIRECTORY, prefix, transactions)
