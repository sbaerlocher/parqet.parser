...
def process_deposits_withdrawals(transactions, timezone="Europe/Zurich"):
    ...
    for idx, transaction in enumerate(transactions):
        try:
            ...
            if not formatted_transaction["datetime"] or not formatted_transaction["amount"]:
                raise ValueError(f"Invalid transaction data: {formatted_transaction}")

            formatted_transactions.append(formatted_transaction)
        except Exception as error:
            logging.error(f"Error processing transaction with datetime {transaction.get('datetime', 'Unknown')}: {error}")

    if len(formatted_transactions) <= 10:
        logging.debug(f"Formatted deposit/withdrawal transactions: {formatted_transactions}")
    else:
