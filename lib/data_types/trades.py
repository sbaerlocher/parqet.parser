import logging
from lib.common.utilities import format_number_for_reading, process_datetime_to_utc, calculate_price
...
def extract_transaction_financials(transaction):
    ...
    try:
        ...
        logging.debug(f"Formatting financials: amount={amount}, shares={shares}, price={calculated_price}")
...
def format_transaction(transaction, localized_datetime):
    ...
    try:
        ...
        logging.error(f"Error formatting transaction with identifier {transaction.get('isin_code', 'Unknown')} and datetime {transaction.get('datetime', 'Unknown')}: {e}")
...
def process_trades(transactions):
    ...
    for idx, transaction in enumerate(transactions):
        try:
            ...
            logging.error(f"Error processing trade transaction with identifier {transaction.get('isin_code', 'Unknown')} and datetime {transaction.get('datetime', 'Unknown')}: {error}")
