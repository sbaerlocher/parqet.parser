...
class RelaiBroker:
    ...
    def process_transactions(self, transactions, file_path=None):
        ...
        categorized_data = {"trades": [], "deposits_withdrawals": []}
        ...
        for idx, tx in transactions.items():
            try:
                ...
                iban = tx.get("Destination") or (
                    self.extract_iban_from_filename(file_path)
                    if file_path
                    else "unknown"
                )
                ...
                tx.update(
                    {
                        "datetime": date_value,
                        "total_amount": standardize_number(
                            tx.get("Fiat Amount (excl. fees)", 0)
                        )
                        - standardize_number(tx.get("Fee", 0)),
                    ...
                    categorized_data["deposits_withdrawals"].append(
                        {
                            ...
                            "type": "TransferIn",
                            "iban": iban,
                            "holding": "hld_682a056a33c11e34be888b48",
                        }
                    )
                ...
                logging.error(
                    f"Error processing transaction {idx}: {tx}, Error: {error}"
                )
